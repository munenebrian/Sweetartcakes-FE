...
@Component({
  selector: 'app-bridalshower-details',
  templateUrl: './bridalshower-details.component.html',
  styleUrls: ['./bridalshower-details.component.css']
})
export class BridalshowerDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.ocassionalcakedetailsService.getBridalShowerCakeDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/bridalShowerCakes/').subscribe(response => {
