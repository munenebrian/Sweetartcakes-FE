...
@Component({
  selector: 'app-baptismal-details',
  templateUrl: './baptismal-details.component.html',
  styleUrls: ['./baptismal-details.component.css']
})
export class BaptismalDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.ocassionalcakedetailsService.getBaptismalCakeDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/baptismalCakes/').subscribe(response => {
