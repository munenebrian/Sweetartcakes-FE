...
@Component({
  selector: 'app-christmass-details',
  templateUrl: './christmass-details.component.html',
  styleUrls: ['./christmass-details.component.css']
})
export class ChristmassDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.ocassionalcakedetailsService.getChristmasCakeDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/christmasCakes/').subscribe(response => {
