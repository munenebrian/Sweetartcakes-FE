...
@Component({
  selector: 'app-graduation-details',
  templateUrl: './graduation-details.component.html',
  styleUrls: ['./graduation-details.component.css']
})
export class GraduationDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.ocassionalcakedetailsService.getGraduationCakeDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/graduationCakes/').subscribe(response => {
