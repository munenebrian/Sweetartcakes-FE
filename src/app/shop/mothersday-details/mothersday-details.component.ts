...
@Component({
  selector: 'app-mothersday-details',
  templateUrl: './mothersday-details.component.html',
  styleUrls: ['./mothersday-details.component.css']
})
export class MothersdayDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.ocassionalcakedetailsService.getMothersDayCakeDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/mothersDayCakes/').subscribe(response => {
