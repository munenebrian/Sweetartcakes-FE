...
@Component({
  selector: 'app-ruracio-details',
  templateUrl: './ruracio-details.component.html',
  styleUrls: ['./ruracio-details.component.css']
})
export class RuracioDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.productdetailsService.getRuracioDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/ruracioCakes/').subscribe(response => {
