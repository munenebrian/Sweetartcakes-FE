...
@Component({
  selector: 'app-christening-details',
  templateUrl: './christening-details.component.html',
  styleUrls: ['./christening-details.component.css']
})
export class ChristeningDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.ocassionalcakedetailsService.getChristeningCakeDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/christeningCakes/').subscribe(response => {
