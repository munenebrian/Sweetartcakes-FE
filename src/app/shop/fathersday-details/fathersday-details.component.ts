...
@Component({
  selector: 'app-fathersday-details',
  templateUrl: './fathersday-details.component.html',
  styleUrls: ['./fathersday-details.component.css']
})
export class FathersdayDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.route.params.subscribe(
      data =>{
        this.id = data['id'];
        console.log(this.id);
        this.ocassionalcakedetailsService.getFathersDayCakeDetails(this.id).subscribe(
          datax =>{
  ...
  getData() {
    this.http.get<Product[]>('https://sweetartcakes-be-production.up.railway.app/fathersDayCakes/').subscribe(response => {
